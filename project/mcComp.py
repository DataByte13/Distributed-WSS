class Sensor():
    def __init__(self, k: int, sensor_id: 2):
        ...
        self.targetTracked = {}
        # {tracker_id : [target1 , ...]}
        self.node_tracking_status = {2: ['t2', 't1'], 3: ['t2', 't3', 't4'], 1: ['t2', 't3', 't1'], 4: ['t3', 't4'] , 6:['t2', 't1']}
        self.assignment = {}
        self.K = k
    ...
    def unassigned_Node(self, assignment, targetTracked):
        ...
        for element in self.node_tracking_status.keys():
            if element not in list(targetTracked.values()) and element not in assignment.keys():
...
inst = Sensor(3, 2)
inst.local_tree = {3: [], 1: [], 6: [], 2: []}
inst.is_haveBetter_tree({2: ["X"], 3: ["X"], 6: ["X"], 1: ["X"]}, {3: ["t2"], 1: [], 6: [], 2: []})
